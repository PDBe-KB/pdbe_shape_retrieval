...
def export_correspondence_rotation_gif(source_path,target_path,map_path,R_init,out_gif,n_frames=60,fps=15,axis='y'):
    axis_vec = {
        "x": np.array([1, 0, 0]),
        "y": np.array([0, 1, 0]),
        "z": np.array([0, 0, 1]),
    ...
    # Load meshes
    src = trimesh.load(source_path)
    tgt = trimesh.load(target_path)

    verts_src = np.asarray(src.vertices)
    verts_tgt = np.asarray(tgt.vertices)

    faces_src = np.hstack([np.full((len(src.faces), 1), 3), src.faces])
    faces_tgt = np.hstack([np.full((len(tgt.faces), 1), 3), tgt.faces])
    ...
    # --------------------------------------------------
    # FIX RELATIVE CONFIGURATION ONCE
    # --------------------------------------------------

    verts_tgt_aligned = apply_rotation(verts_tgt, R_init)
    
    x_shift = (
        np.max(verts_src[:, 0])
        - np.min(verts_tgt_aligned[:, 0])
        + 1.0
    )

    verts_tgt_aligned[:, 0] += x_shift

    # --------------------------------------------------
    # GLOBAL ROTATION LOOP
    # --------------------------------------------------
    ...
    for angle in np.linspace(0, 2 * np.pi, n_frames, endpoint=False):
        ...
        R = trimesh.transformations.rotation_matrix(angle, axis_vec)[:3, :3]

        verts_src_rot = apply_rotation(verts_src, R)
        verts_tgt_rot = apply_rotation(verts_tgt_aligned, R)

        src_mesh = pv.PolyData(verts_src_rot, faces_src)
        src_mesh["colors"] = (colors_src * 255).astype(np.uint8)

        tgt_mesh = pv.PolyData(verts_tgt_rot, faces_tgt)
        tgt_mesh["colors"] = (colors_tgt * 255).astype(np.uint8)

        plotter.add_mesh(src_mesh, scalars="colors", rgb=True)
        plotter.add_mesh(tgt_mesh, scalars="colors", rgb=True)
...
def visualize_correspondence_aligned(source_path, target_path, map_path, R,t):
    src_mesh = trimesh.load(source_path)
    tgt_mesh = trimesh.load(target_path)
    ...
    verts_tgt_aligned = verts_tgt @ R.T + t

    # Color transfer
    colors_src = normalize_colors(verts_src)
    colors_tgt = colors_src[correspondence]

    # Plot aligned (overlapping)
    double_plot_with_correspondences(
        verts_src, faces_src,
        verts_tgt_aligned, faces_tgt,
        colors_src, colors_tgt
    )