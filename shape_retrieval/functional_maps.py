...
def set_FM_model_parameters(
    ...
    process_params = {
        "n_ev": (
            n_ev,
            n_ev,
        ),  # n_ev: (k1, k2) tuple - with the least number of Laplacian eigenvalues to consider.
...
def calculate_functional_maps(
    ...
    *,
    ...
    if fm_config.refine is None:
        ...
        model.change_FM_type(
            "classic"
        )  # We refine the first computed map, not the icp-refined one
...
def calculate_scalable_functional_maps(
    ...
    p2p_21_sub_init = knn_query(
        mesh1.vertices[sub1], mesh2.vertices[sub2], k=1, n_jobs=n_cpus
    )
...
def calculate_functional_maps_chem(
    ...
    *,
    ...
    logger.info("cpus used: %s", fm_config.n_cpus)
    model.fit_othdescr(
        descr1, descr2, **fm_config.fit_params(), verbose=fm_config.verbose
    )
