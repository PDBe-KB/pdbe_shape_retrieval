...
import shape_utils.pyFM_pdbe.signatures as sg
import shape_utils.pyFM_pdbe.optimize as opt_func
import shape_utils.pyFM_pdbe.refine
...
class FunctionalMapping:
    ...
    def preprocess(self, n_ev=(50,50), n_descr=100, descr_type='WKS', landmarks=None, subsample_step=1, k_process=None, verbose=False):
        ...
        if descr_type == 'HKS':
            ...
            self.descr1=results_HKS[0][0]
            self.descr2=results_HKS[1][0]
            self.energylist =results_HKS[0][1]
            
            self.descr1,self.energylist = sg.mesh_HKS(self.mesh1, n_descr, k=self.k1)  # (N1, n_descr)
            self.descr2,self.energylist = sg.mesh_HKS(self.mesh2, n_descr, k=self.k2)  # (N2, n_descr)

            if use_lm:
                lm_descr1, self.energylist = sg.mesh_HKS(self.mesh1, n_descr,landmarks=lmks1, k=self.k1)  # (N1, p*n_descr)
                lm_descr2, self.energylist = sg.mesh_HKS(self.mesh2, n_descr, landmarks=lmks2, k=self.k2)  # (N2, p*n_descr)
            ...
            #if not use_lm:
            start = time.time()
            inputs_mesh_WKS = [(self.mesh1,n_descr,landmarks,self.k1),(self.mesh2, n_descr,landmarks,self.k2)]
            with Pool() as pool:
                results_WKS=pool.starmap(sg.mesh_WKS,inputs_mesh_WKS)
            
            #self.descr1,self.energylist = sg.mesh_WKS(self.mesh1, n_descr, k=self.k1)  # (N1, n_descr)
            #self.descr2,self.energylist = sg.mesh_WKS(self.mesh2, n_descr, k=self.k2)  # (N2, n_descr)

            totaltime=time.time()-start

            print(f"Finished analysis of descriptors in {totaltime} seconds")    

            self.descr1=results_WKS[0][0]
            self.descr2=results_WKS[1][0]
            self.energylist =results_WKS[0][1]
            
            if use_lm:
                lm_descr1, self.energylist= sg.mesh_WKS(self.mesh1, n_descr, landmarks=lmks1, k=self.k1)  # (N1, p*n_descr)
                lm_descr2, self.energylist= sg.mesh_WKS(self.mesh2, n_descr, landmarks=lmks2, k=self.k2)  # (N2, p*n_descr)
        ...
        self.energylist = self.energylist[::subsample_step]
    ...
    def icp_refine(self, nit=10, tol=None, use_adj=False, overwrite=True, verbose=False, n_jobs=1):
        ...
        self._FM_icp = shape_utils.pyFM_pdbe.refine.mesh_icp_refine(self.FM, self.mesh1, self.mesh2, nit=nit, tol=tol,
                                                   use_adj=use_adj, n_jobs=n_jobs, verbose=verbose)

        if overwrite:
            self.FM_type = 'icp'

    def zoomout_refine(self, nit=10, step=1, subsample=None, use_ANN=False, overwrite=True, n_jobs=1,verbose=False):
        """
        ...
        self._FM_zo = shape_utils.pyFM_pdbe.refine.mesh_zoomout_refine(self.FM, self.mesh1, self.mesh2, nit,
                                                      step=step, subsample=sub, use_ANN=use_ANN,n_jobs=n_jobs,verbose=verbose)
        if overwrite:
