...
def auto_HKS(evals, evects, num_T, landmarks=None, scaled=True):
    ...
    if landmarks is None:
        return HKS(abs_ev, evects, t_list, scaled=scaled), t_list
    else:
        return lm_HKS(abs_ev, evects, landmarks, t_list, scaled=scaled), t_list
...
def mesh_HKS(mesh, num_T, landmarks=None, k=None):
    ...
    HKS_descriptor,t_list = auto_HKS(mesh.eigenvalues[:k], mesh.eigenvectors[:,:k], num_T, landmarks=landmarks, scaled=True)
    return HKS_descriptor, t_list