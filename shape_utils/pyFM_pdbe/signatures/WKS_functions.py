...
def auto_WKS(evals, evects, num_E, landmarks=None, scaled=True):
    ...
    if landmarks is None:
        return WKS(abs_ev, evects, energy_list, sigma, scaled=scaled),energy_list
    else:
        return lm_WKS(abs_ev, evects, landmarks, energy_list, sigma, scaled=scaled),energy_list
...
def mesh_WKS(mesh, num_E, landmarks=None, k=None):
    ...
    wks_descriptors,energylist = auto_WKS(mesh.eigenvalues[:k], mesh.eigenvectors[:, :k], num_E, landmarks=landmarks, scaled=True)
    #return auto_WKS(mesh.eigenvalues[:k], mesh.eigenvectors[:, :k], num_E, landmarks=landmarks, scaled=True)
    return wks_descriptors,energylist